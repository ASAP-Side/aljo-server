...
async function findAlarmDateTime(req, res) {
...
async function createAlarmDateTime(req, res) {
    const now = new Date();
    const parsedStartDate = `${now.getFullYear()}-${now.getMonth() + 1}-${now.getDate()}`;
...
async function destroyAlarmDateTime(req, res) {
...
module.exports = { findAlarmDateTime, createAlarmDateTime, destroyAlarmDateTime };
